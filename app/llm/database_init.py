...
class DatabaseInitializer:
    def __init__(self, redis_client: redis_client.RedisClient):
        self.redis_client = redis_client

    async def initialize_database(self):
        """Initialize Redis with sample data from LLM"""
        # Check if database is already initialized
        if self.redis_client.is_initialized():
        ...
        redis_commands = init_data.get("redis_commands", [])
        redis_results = self.redis_client.execute_commands(redis_commands)
        
        # Mark database as initialized
        self.redis_client.mark_initialized()
        
        logger.info("Database initialization complete")
        logger.debug("Initialization results: %s", redis_results)